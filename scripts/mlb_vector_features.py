...
def prepare_statcast_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    xwoba_estimate = pd.to_numeric(df["estimated_woba_using_speedangle"], errors="coerce")
    woba_fallback = pd.to_numeric(df["woba_value"], errors="coerce")
    df["xwoba_value"] = xwoba_estimate.where(xwoba_estimate.notna(), woba_fallback)
